...
export async function POST(reqs) {
  try {
    ...
    if (!seller) {
      return NextResponse.json({ mensaje: 'El usuario no es un vendedor' }, { status: 403 });
    }
    ...
    for(const req of requests){
      ...
      delete body.day;
        ...
        try {
            ...
            console.error(`Error creating schedule object for request ${i}:`, error);
            return NextResponse.json({ message: `Error creating schedule object for request ${i}`, error: error.message }, { status: 500 });
        }
        try {
            await newSchedule.save();
        } catch (error) {
            console.error(`Error saving schedule for request ${i}:`, error);
            return NextResponse.json({ message: `Error saving schedule for request ${i}`, error: error.message }, { status: 500 });
        }
        i+=1;
    }
    /*
    // Return a successful response*/
    return NextResponse.json({ message: 'Schedules created successfully' }, { status: 201 });
  }
  } catch (error) {
    // Handle errors and return a response with the message
    return NextResponse.json({ message: 'Error creating schedules', error: error.message }, { status: 500 });
  }
}