...
// `response.sellerId._id !== seller?._id` to decide whether the visitor owned
// it. Both jobs moved to the server (see getProductForEdit): the product
// arrives as a prop, already known to belong to this session, so there is no
// load left to fail and no ownership check that can be skipped because the
// request 500'd before returning.
//
// The approval gate stays here, unchanged. Adding, listing and editing
// products are all gated the same way - useCheckSeller sends a seller who is
// not approved to /antojos/sellers/approving - and whether an unapproved
// seller should be allowed to edit is a product decision, not part of fixing
// a 500.
    ...
    <div className='flex flex-col h-dvh relative'>
      ...
      <div className='h-3/4 bg-[#393939]'>
        <div className='bg-base-100 text-base-content rounded-t-3xl h-max w-full absolute px-6 pt-6 pb-16'>
          {/* T-97: a failed save or delete used to replace the whole screen
              with `<p>{error}</p>` - the same bare line of text F28 was
